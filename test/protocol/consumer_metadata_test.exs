defmodule KafkaEx.Protocol.ConsumerMetadata.Test do
  ...
  test "Response.broker_for_consumer_group returns correct coordinator_broker" do
    ...
    socket = %KafkaEx.AbstractSocket.Socket{socket: fake_socket, ssl: false}
    ...
    brokers = [
        %KafkaEx.Protocol.Metadata.Broker{host: "192.168.0.1", port: 9092, node_id: 42, socket: socket},
        %KafkaEx.Protocol.Metadata.Broker{host: "192.168.59.103", port: 49162, node_id: 49162, socket: socket}
    ]

    assert KafkaEx.Protocol.ConsumerMetadata.Response.broker_for_consumer_group(brokers, consumer_group_metadata) == %KafkaEx.Protocol.Metadata.Broker{host: "192.168.59.103", port: 49162, node_id: 49162, socket: socket}
  ...
  test "Response.broker_for_consumer_group returns 'nil' when the broker's socket is closed" do
    ...
    brokers = [
        ...
        %KafkaEx.Protocol.Metadata.Broker{host: "192.168.0.103", port: 9092, socket: socket}
    ]
